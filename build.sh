...
if [ ${SDKJS_DIR+x} ] ; then
  ...
  git clone --depth 1 --branch $OO_BRANCH https://github.com/ONLYOFFICE/sdkjs.git
fi

if [ ${WEB_APPS_DIR+x} ] ; then
  copy_git "$WEB_APPS_DIR" "$WORK_DIR/web-apps"
else
  git clone --depth 1 --branch $OO_BRANCH https://github.com/ONLYOFFICE/web-apps.git
fi

if [ ${ONLYOFFICE_EDITOR_DIR+x} ] ; then
  copy_git "$ONLYOFFICE_EDITOR_DIR" "$WORK_DIR/onlyoffice-editor"
else
  git clone --depth 1 --branch "$EDITOR_BRANCH" g
